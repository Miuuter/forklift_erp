...
当前共 17 项前端单元测试，覆盖请求 ID、下载错误上下文、批量请求按 ID 排序/版本传递，以及压力测试在十万级延迟样本下的汇总与分位数计算。
...
当前工作树默认 Java 测试为 226 项；2026-08-02 的 `clean verify` 结果为
`226/226`，无失败、错误或跳过。JaCoCo 指令/分支/行/方法覆盖率分别为
34.8656% / 29.7924% / 36.8390% / 36.5245%，均高于仓库基线。
...
当前 Playwright 为 7 项核心模块入口冒烟和 1 项客户新增写入流程；未提供账号时全部明确跳过，配置隔离测试环境后才执行真实写入。
